...
class ScrapeData(Model):
    scrape = fields.ForeignKeyField('models.Scrape', on_delete=fields.CASCADE, reference='Scrape.name')
...
class DBClient:
    ...
    async def get_24h_data(self, scrape: Scrape) -> pd.DataFrame:
        ...
        for data in data_list:
            row = OrderedDict([('datetime', data.datetime.astimezone(pytz.timezone(TZ)))])
