...
class SecretsManager(object):
    ...
    def create_secret(self, name: str, access_type: AccessType, secret: str) -> bool:
        try:
            obj, created = Secrets.objects.update_or_create(
    ...
    def get_secret(self, name: str) -> Secrets:
        return Secrets.objects.filter(user=self.user, name=name).first()

    def delete_secret(self, name: str) -> bool:
        try:
            asset = Secrets.objects.filter(user=self.user, name=name).first()
            asset.delete()
        except Exception as e:
